...
router.get('/:id', async (req: Request, res: Response) => {
  ...
  let whereStatus = '';
  if (status === 'stockout') whereStatus = 'AND stock_days = 0';
  else if (status === 'slow_moving') whereStatus = 'AND stock_days >= 30 AND stock_qty >= 3';

  try {
    const [rows] = await pool.execute<InventoryRow[]>(
      `SELECT spec_id, stock_qty, stock_days, snapshot_date
         FROM cust_inventory
        WHERE customer_id = ?
          AND snapshot_date = (
