...
exports.getCustomerClass = getCustomerClass;
 ...
 */
async function getCustomerClass(customerId) {
    if (!customerId)
        return 'new';
    try {
        const [rows] = await db_1.default.execute('SELECT joined_at FROM cust_info WHERE customer_id = ?', [customerId]);
