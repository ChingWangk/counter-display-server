...
exports.regularCustomerStrategy = void 0;
 ...
 */
const regularCustomerStrategy = async (ctx) => {
    if (!ctx.customerId) {
    ...
    if (rows.length === 0) {
        throw new types_1.ValidationError('该常规客户暂无库存数据，请确认数据已导入');
    }
    // 构建 spec_id → 最新库存快照
    ...
    let withImages = (0, sortCategories_1.sortCategories)(carriedSpecs);
    // 资源不足时过滤紧俏烟，高价烟保护
