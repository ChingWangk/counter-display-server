 ...
 */
async function generateCounterImage(counter, regularSpecs, regularRows, zonePlacements, priceTagMap, regularLayout, special) {
    ...
    if (coopUnits.length > 0) {
        ...
        for (let r = 0; r < COOP_ROWS; r++) {
            ...
            if (rowUnits.length > 0)
                coopRowSlots.push({ type: 'coop', units: rowUnits });
        }
    }
    // 内嵌红框对:每对宽 2 包 + 组间留 MIN_INTER_GROUP_GAP_PX,bin-packing 到行
    const inlinePairRowSlots = [];
 ...
 */
async function drawCarton(ctx, spec, x, y) {
 ...
 */
async function drawCoopRow(ctx, units, areaStartX, areaW, baseY, priceTagMap) {
    if (units.length === 0)
        return;
    ...
    for (let i = 0; i < n; i++) {
        ...
        for (let k = 0; k < COOP_PACKS_PER_UNIT; k++) {
            ...
            cursor += CELL_W;
            if (k < COOP_PACKS_PER_UNIT - 1)
                cursor += COOP_PACK_GAP_PX;
        }
    }
}
/**
 ...
 */
async function drawInlinePairRow(ctx, pairs, areaStartX, areaW, baseY, priceTagMap) {
    if (pairs.length === 0)
        return;
    const pairW = 2 * CELL_W;
